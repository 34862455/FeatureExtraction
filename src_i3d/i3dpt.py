...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.maxPool3d_4a_3x3 = MaxPool3dTFPadding(
            kernel_size=(3, 3, 3), stride=(1, 2, 2), padding='SAME') #reduce downsampling
        ...
        self.maxPool3d_5a_2x2 = MaxPool3dTFPadding(
            kernel_size=(2, 2, 2), stride=(1, 2, 2), padding='SAME') #reduce downsampling
    ...
    def extract_features(self, inp):
        # print(f"[DEBUG] Input temporal length: {inp.shape[2]}")

        # Same as forward() up to avg_pool
        out = self.conv3d_1a_7x7(inp)
        out = self.maxPool3d_2a_3x3(out)
        out = self.conv3d_2b_1x1(out)
        out = self.conv3d_2c_3x3(out)
        ...
        out = self.mixed_5b(out)
        out = self.mixed_5c(out)
        out = torch.mean(out, dim=[3, 4], keepdim=True)  # keep (T) intact

        output_T = out.shape[2]

        # print(f"[DEBUG] Output temporal length: {out.shape[2]}")
        return out, output_T# Return pooled features
