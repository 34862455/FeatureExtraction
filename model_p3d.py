...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))                              # pooling layer for res5.
    ...
    def extract_features(self, x):
        """
        Returns temporally sliced features: shape (T, 2048)
        """
        x = self.conv1_custom(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.layer4(x)  # -> (B*T, 2048, H', W')
        x = self.avgpool(x)  # -> (B*T, 2048, 1, 1)
        x = x.view(B, T, 2048)  # -> (B, T, 2048)

        return x.squeeze(0)  # Return (T, 2048) if B=1
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    if pretrained==True:
        if modality=='RGB':
            pretrained_file='checkpoints/p3d_rgb_199.checkpoint.pth.tar'
        elif modality=='Flow':
